...
| `camera_car_drive.py` | 有线相机小车键盘控制。默认 COM21、ID4 前中 / ID5 后左 / ID6 后右 |
...
| `tests/` | 24 个单测（运动学 + 编码，跑法见下） |

## 快速开始

> 以下命令在 `YuriChassis/` 目录下执行（从仓库根先进 `cd YuriChassis`）。

```bash
# 直接跑，键盘控制：w前 s后 a左移 d右移 z自旋 空格停 q退
# 需：ID7/8/9 舵机已设好、12V 供电、COM5 未被占用
..\lerobot_venv312\Scripts\python.exe kiwi_drive.py
```
...
默认轮轴角度为 `0° / 225° / 135°`，默认前中轮反向、后左轮反向、后右轮正向；前进时后左/后右同向，后退相反，前中轮保持 0 速。如果实际舵机 ID 或轮轴方向不同，用
`--front-id`、`--rear-left-id`、`--rear-right-id` 和对应角度参数覆盖。

GUI 中提供 `前中反向`、`后左反向`、`后右反向` 三个开关；当前默认前中、后左勾选，后右不勾选。
...
```bash
..\lerobot_venv312\Scripts\python.exe -m unittest discover -s tests -v
```
