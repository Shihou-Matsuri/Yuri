...
> 再动任何代码。仓库根：`C:\Users\21209\Desktop\Yuri`（git 凭据已配，s0lo201）。
...
- Python venv **在仓库根 `lerobot_venv312/`**（3.12.14 + lerobot 0.6.1 + pyserial + fastapi/uvicorn/pywebview/pyinstaller）。
  **python.exe 是 venvlauncher**，曾两次 0 字节损坏：修复 = 从 uv python `cpython-3.13.3-...\Lib\venv\scripts\nt\venvlauncher.exe` 复制改名。
- 串口：主动臂 COM7、ESP32 COM8(115200)、有线相机车默认 COM21(1M)。
- lerobot 源码在仓库外 editable（`pip show lerobot` 查位置）；`E:\Anaconda\...` 是**合作者 conda 示例，勿改**。
...
C:\Users\21209\Desktop\Yuri\lerobot_venv312\Scripts\python.exe backend\main.py --mock   :: mock 演示
...
- `build/`、`dist/`、`*.spec`、`lerobot_venv312/` 均已 gitignore；新增产物注意保持忽略。
